...
/**
 * Recursively runs the supplied functor on any files that are found (normal or
 * otherwise)
 *
 * @param root_path           the directory in which to start the recursion
 * @param file_name_handler   the functor which will receive the file name of
 * each file found
 */
void recurse_files(const char* root_path, const std::function<void(const char*)>& file_name_handler) {
  // if its a directory that exists
  filesystem::path root(root_path);
  if (filesystem::exists(root) && filesystem::is_directory(root)) {
    // iterate over all the things in it
    for (filesystem::recursive_directory_iterator i(root), end; i != end; ++i) {
      // if this thing is a file
      // if (i->is_regular_file() || i->is_symlink()) {
      // give it to the functor
      file_name_handler(i->path().c_str());
      //}
    }
  }
}
...
int main(int argc, char** argv) {
  recurse_files(argv[1], [](const char* path_name) { std::cout << path_name << std::endl; });
  return 0;
}
